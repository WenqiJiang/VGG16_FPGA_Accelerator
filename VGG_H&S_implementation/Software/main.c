            ...
            for (int point = 0; point < width * (width - 2); point++)
            {
                ...
                */  
                    if (padding != in)
                    {
                        ...
                        point_enable(1, 0, 0, partial_enable, conv_num, sub_channel, OUTPUT_SRAM_PE_ARRAY + 64 * 2 + point % width * 64 * 2);
                    }
                    else
                    {
                        point_enable(1, 0, 1, partial_enable, conv_num, sub_channel, OUTPUT_SRAM_PE_ARRAY + 64 * 2 + point % width * 64 * 2);
                    }
...
void point_enable(int output_enable, int pool_enable, int relu_enable, int partial_enable,
                  int conv_num, int layer, int output_addr_onchip)
{
              ...
              int bias_addr, int conv_num)
{
    ...
    point_enable(output_enable, pool_enable, relu_enable, partial_enable, conv_num, channel, output_addr_onchip_rcv + 64 * 2 + point/2 % width * 64 * 2);
    // Receive the signal from FPGA for current state
    while (*(CONTROLLER_status_ptr + 1) == 1) 
    {
        result_signal = *(CONTROLLER_status_ptr + 1);
